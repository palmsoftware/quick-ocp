...
# Returns success (0) if the given API group has at least one resource discoverable
api_group_has_resources() {
    local group="$1"
    local out
    out=$(oc api-resources --api-group="$group" --no-headers 2>/dev/null | head -1 || true)
    [[ -n "$out" ]]
}

# Wait for discovery of a newly-created API group (via CRDs) with retries
wait_for_api_group() {
    local group="$1"; local retries="${2:-6}"; local sleep_s="${3:-5}"
    local i=0
    while (( i < retries )); do
        if api_group_has_resources "$group"; then
            return 0
        fi
        sleep "$sleep_s"; i=$((i+1))
    done
...
check_api_groups() {
    ...
    for group in "${api_groups[@]}"; do
        if [[ "$group" == "config.openshift.io" ]]; then
            # config.openshift.io is created by spoof script (CRDs). Give discovery some time in CI.
            if wait_for_api_group "$group" 12 5; then
                print_success "✅ API Group: $group"
                PASSED_TESTS=$((PASSED_TESTS + 1))
            else
                print_error "❌ API Group: $group"
                FAILED_TESTS=$((FAILED_TESTS + 1))
            fi
            TOTAL_TESTS=$((TOTAL_TESTS + 1))
        else
...
check_project_resources() {
    ...
    # Check for microshift namespaces (including openshift-marketplace which is created by spoof script)
    ...
    # Note: Skip namespaces that don't exist in microshift
    print_info "Skipping microshift-incompatible namespaces: openshift-ovn-kubernetes, openshift-route-controller-manager, openshift-service-ca, openshift-storage"
}
